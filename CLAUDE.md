...
A PR is not ready until all three hold. They are not style preferences, and none of them is waived
because a change is small or because an agent is confident about it.

1. **CI is green.** `dotnet-ci.yml`, every job. A PR from a fork does not run CI until a maintainer
   approves it (`gh api -X POST repos/maccoss/skyline-prism/actions/runs/<id>/approve`) - an
   unapproved run shows as `action_required` with a 0s duration, which is not a pass.
2. **`/code-review` has been run on the branch, and its findings are addressed or answered.**
   Run it yourself before asking anyone else to look. "Addressed" means fixed or explicitly
   rejected with a reason in the PR or the commit message - a finding that is simply not mentioned
   reads as one nobody looked at. Reviews are wrong sometimes: verify before acting, and say so
   when you reject one.
3. **A new algorithm is pinned to a golden, and the golden names its source.** See below - this is
   the gate that exists because this is a measurement tool.

### New algorithms must be pinned to a golden

**Any change that computes a quantity a user will read - a statistic, a normalization, a rollup, a
correction, a score - ships with a committed golden, or it does not ship.** A test that asserts your
implementation agrees with itself proves only that it is deterministic.

The golden goes in `dotnet/tests/fixtures/`, generated by a **checked-in generator** that imports
nothing from PRISM, and it must record **where its numbers came from** in a `reference` field that
names the actual library and call:

```json
{
  "reference": "scipy.stats.ttest_ind(equal_var=False) and inmoose.limma.squeezeVar",
  "note": "what the cases cover and any convention that had to be chosen",
  "cases": [ ... ]
}
```

`dotnet/tests/fixtures/differential/generate.py` is the pattern to copy. Its header pins exact
library versions (PEP 723), it writes floats as **strings** in shortest round-trip form so the
fixture does not depend on a JSON parser's float handling, and each generator function documents any
place the reference offers a choice (scipy's two rank tests disagree with each other about the
continuity correction, and PRISM follows each one's own convention - which is written down, because
copying one to the other silently shifted every signed-rank p-value by 10-20%).

Choosing the source, in order of preference:

1. **A published reference implementation of the same method** - scipy, statsmodels, numpy, inmoose
   (limma), R via a recorded transcript. This is the strongest evidence and is what "pinned"
   normally means.
2. **The lab's `proteomics-toolkit`, when it is the definition of the estimator** - as for the
   intensity-trend prior, where the toolkit *is* what PRISM is trying to match. Say so in the
   `reference` field, because this is deliberately an exception to "the generator imports nothing
   from PRISM's world": the golden then shares an author with the thing under test, and a reader
   must not mistake it for an independent check.
3. **A first-principles construction with the algebra written out** - acceptable only where no
   implementation exists (the Firth-penalized LRT is referenced this way, by maximizing the same
   penalized likelihood with a derivative-free optimizer: a different algorithm reaching the same
   fixed point).

Also required with the golden:

- **A stated tolerance, and why it is that number.** Closed-form arithmetic agrees to ~1e-13;
  anything behind an iterative solve agrees to that solver's tolerance and no better. A uniform
  `1e-6` everywhere tells a reader nothing and hides the one quantity that should have been exact.
- **A note on where PRISM and the reference deliberately differ**, if they do, with the reason.
- **Regenerating a golden to make a test pass is a red flag, not a fix.** A digest or golden failure
  is the gate working. Find out which quantity moved and why before even considering regeneration,
  and never regenerate the whole fixture directory to settle one case - library drift will silently
  rewrite the others (running the full differential generator here rewrote 7 of 10 pre-existing
  goldens purely from a numpy/scipy version difference).
...
The golden is covered above and is a gate. The rest of what a measurement change owes a reader:
...
- Gates 1 and 2 above - green CI, `/code-review` run and answered - are checked here, not assumed.
...
### Working on PRISM with an agent

The gates above do not change when an agent writes the code. What changes is which mistakes are
likely, so this is what to watch for. Every example below actually happened in this repository.

**A plausible number is the failure mode.** An agent will produce code that runs, returns finite
values of the right magnitude, and is wrong. The removed MS2 signal accounting divided a sum of peak
areas by a summed total ion current and reported a "coverage percentage" that was a ratio carrying
units of time - it looked entirely reasonable, and the hazard was documented on its own enum at the
time. Nothing in the output said otherwise. **The golden is the only thing that catches this class
of error**, which is why it is a gate rather than a recommendation.

**Ask where a number came from, not whether it is right.** "Does this look correct?" invites
agreement. "Which reference produced this expected value, and what does its `reference` field say?"
does not. If the answer is "it matches our implementation", there is no golden.

**Measure rather than accept a performance claim.** Absolute throughput figures from this repo have
been wrong more than once because other software was running - an identical configuration measured
2.07 and 3.70 minutes hours apart, and the first explanation reached for was page-cache warmth
rather than the process list. Prefer ratios, compare against the previous release's binary built
from its tag in a `git worktree`, and **assert both arms did the same work**: one arm here once
looked 3.12x faster because it accumulated an index instead of the values, and rows and peptides
matched so the correctness check passed it. `dotnet/bench/Stage2Bench` does all of this.

**A GUI change is unverified until the app has been launched.** `SkylinePrism.App` is Windows-only,
so an agent on Linux cannot build it at all, and even on Windows a compiling XAML change is not a
working one. A `SelectionChanged` handler on a combo preselected in XAML crashed the tool on startup
here and passed every test. Launch it, drive the pane, and read the status line. If an agent says a
WPF change is verified, ask what it saw on screen.

**Findings from a review agent are claims, not facts.** They are usually right and occasionally
confidently wrong. Verify before acting - one review here asserted that removed methodology caveats
survived nowhere in the docs, when they were in `docs/skyline-tool.md`. Rejecting a finding is fine;
rejecting it silently is not, because the next reader cannot tell it was considered.

**Watch for the standard being quietly lowered.** The shapes to refuse:

- regenerating a golden so a test passes, rather than explaining which quantity moved;
- widening a tolerance to accommodate a change, rather than justifying the new number;
- `try`/`catch` that swallows the failure a test was about to find;
- clamping a value that is impossible, which turns a visible defect into a plausible reading
  (`IonAccountingRow.Exceeded` exists so a fraction over 1 is refused rather than clamped);
- adding a second implementation of something the repo already computes, because extending the
  existing one looked harder.

**Say what is not done.** A change that works for the case in front of it and silently degrades on
another is worse than one that refuses. PRISM's habit is to name what it could not honor in the
status line and in the result's messages - an unhonored covariate, a prior that could not be fitted,
subjects dropped from a paired design. Follow it: an agent that reports "done" without listing what
it skipped has made the next person's job harder, not easier.

**Keep this file honest.** `CLAUDE.md` is long because nearly every caution in it is the residue of
a specific mistake, and its value is that each one is still true. If you find a passage that
contradicts the code, fix the passage in the same PR - a stale doc comment is a defect here, not a
nit, since most of this repository's hard-won knowledge lives in prose.
